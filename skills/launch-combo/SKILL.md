...
In the normal path the runner writes every event. `combo-chen emit -n <comboId> <event> [--field k=v...]` is the recovery lever for when a real-world fact happened but the runner died before journaling it, so the combo is frozen in the wrong phase. The `bin/cb-*.sh` tooling also provides `cb-emit.sh` (validated append), `cb-wait.sh` (poll for events), `cb-run-state.sh` (deterministic phase folding), and the P2 tmux spawn scripts (`cb-tmux.sh`, `cb-agent-spawn.sh`, `cb-send.sh`, `cb-peek.sh`, `cb-status.sh`) for the fixed role topology. Reach for emit only after `combo-chen resume` has run and either punted to a salvage state or does not cover the fact you need to record, and only after diagnosis (`combo-chen status`, `combo-chen events`, `gh pr view`, `no-mistakes axi status`) confirms the fact is true and the journal is the only thing out of date. Never emit to fabricate a state that did not happen.
