...
# Caller must ensure the target is live; capture failure is not "cleared".
...
cb_tmux_send_fail_dead() {
  echo "cb-send: target pane dead or missing" >&2
  return 1
}

# Success only when still live after composer-clear classification.
cb_tmux_send_success_if_live() {
  cb_tmux_target_live "$1" || cb_tmux_send_fail_dead
}

cb_tmux_send_verified() {
  # Type once; Enter-only retries.
  # live + not pending + final live → success; dead/missing → error; live + pending → retry/fail.
  target=$1 text=$2
  retries=${CB_SEND_RETRIES:-3}
  sleep_s=${CB_SEND_SLEEP:-0.4}
  settle=${CB_SEND_SETTLE:-0.1}
  case "$retries" in ''|*[!0-9]*|0) retries=3 ;; esac
  cb_tmux_target_live "$target" || cb_tmux_send_fail_dead
  ...
  while [ "$i" -lt "$retries" ]; do
    ...
    if ! cb_tmux_composer_pending "$target" "$text"; then
      cb_tmux_send_success_if_live "$target"
      return $?
    fi
  done
  cb_tmux_target_live "$target" || cb_tmux_send_fail_dead
