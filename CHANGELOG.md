...
* **structure:** Re-split `src/cli` by domain so `cli/` keeps only the router (`main.ts`) and adapters (`args.ts`, `display.ts`). Domain orchestration moves to dedicated modules (`src/director/`, `src/gate/`, `src/github/`, `src/launch/`, `src/lifecycle/`, `src/reporting/`). Duplicate basenames across trees are eliminated with explicit renames (`handlers.ts` → `*-handlers.ts`, `work-plan.ts` → `persisted-work-plan.ts`, `coder.ts` → `coder-invocation.ts`, etc.). Fixes [#285](https://github.com/thellmwhisperer/combo-chen/issues/285).
...
* **reviewer:** Harden plain-command safety validation to reject unsupported shell syntax (unescaped quotes, background commands, command substitution, shell grouping, comments, trailing escapes).
