...
* **pr-labels:** Make combo PR label projection single-writer and idempotent. Read-only commands (`status`, `status --deep`, `status --deep --all`) no longer mutate GitHub labels or journal `pr_labels_updated` events; only the in-process supervisor tick writes labels. Fixes [#241](https://github.com/thellmwhisperer/combo-chen/issues/241).
...
* **overture:** Verify declared team identities (`[team]` block in `combo-chen.toml`) before launch. Resolves each role's effective binary → agent → model identity from the tool's own config and hard-fails on mismatch. Resolved identities are journaled as a `team` event and frozen in the config snapshot so the in-process supervisor can flag mid-run identity drift. Undeclared teams keep current behavior. Fixes [#260](https://github.com/thellmwhisperer/combo-chen/issues/260).
* **capsule:** Replace generated runner, gate, and director-watch shell scripts with the TypeScript capsule engine, in-process gate, and event-driven supervisor. The `no-shell-in-ts` tombstone now prevents shell loops, traps, and parsers from returning to TypeScript strings. Fixes [#283](https://github.com/thellmwhisperer/combo-chen/issues/283).
* **review:** Add the bounded local V-C-V verdict loop, schema-versioned verdict artifacts, patch-id LGTM carry-over, deterministic READY agreement, review dossiers, and permanent exit summaries.
* **tui:** Add the Ink/React fleet home with journal-derived threads, durable decision cards, role jumps, and live telemetry.
* **director-skill:** Add the self-contained `direct-combos` operating contract for external agents supervising multi-combo fleets.
* **topology:** Seat capsule-owned coder and reviewer children in their named tmux windows while pane 0 retains process custody; missing seats escalate instead of silently running unseated.
