...
* **status:** marks non-terminal combos with missing tmux sessions as `needs_human reason=tmux_missing` so the stale row remains visible and the operator can investigate instead of seeing a phantom supervised combo ([#90](https://github.com/thellmwhisperer/combo-chen/issues/90))
