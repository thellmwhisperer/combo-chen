...
* **update:** U72-A active combo runtime detection — read-only detector at `src/core/active-runtime.ts` scanning persisted combo state under `COMBO_CHEN_HOME/runs` (`combo.json`, `journal.jsonl`, `runtime-ledger.json` with legacy fallback); returns structured `idle`/`active`/`stale`/`error` status with active combo, stale combo, and detection-error arrays; does not prompt, run tmux/git/gh/no-mistakes commands, write journals, create ledgers, restart daemons, or change update/install targets; fully tested for active, idle, stale, malformed runtime state, mixed error cases, and read-only non-mutation invariants ([#190](https://github.com/thellmwhisperer/combo-chen/issues/190))
